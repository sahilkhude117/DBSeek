...
export const generateQuery = async (input: string) => {
    ...
    try {
        const result = await generateObject({
            ...
            `,
            prompt: `Generate the query neccessary to retrieve the data the user wants: ${input}`,
            schema: z.object({
                query: z.string(),
            }),
        });
...
export const explainQuery = async (input: string, sqlQuery: string) => {
    ...
    try {
        const result = await generateObject({
            model: openai("gpt-4o"),
            schema: z.object({
                explanations: explainationSchema,
            }),
...
export const generateChartConfig = async (
    ...
    try {
        ...
        throw new Error("Failed to generate chart suggetion");
    }
};